...
- (void)viewDidLoad {
  ...
  self.navigationItem.title = NSLocalizedStringFromTable(@"Push Setting", @"RongCloudKit",nil);
    ...
    case 0: {
      switch (indexPath.row) {
        case 0: {
            ...
            cell.leftLabel.text = NSLocalizedStringFromTable(@"Show Push Msg", @"RongCloudKit",nil);
    ...
    case 101:{
      if (switchButton.on == YES) {
        [[RCIMClient sharedRCIMClient].pushProfile
         ...
         success:^{
           dispatch_async(dispatch_get_main_queue(), ^{
             [self AlertMessage:NSLocalizedStringFromTable(@"Setting Success", @"RongCloudKit",nil)];
           });
         } error:^(RCErrorCode status) {
           dispatch_async(dispatch_get_main_queue(), ^{
             [self AlertMessage:NSLocalizedStringFromTable(@"Setting Failure", @"RongCloudKit",nil)];
           });
         }];
      } else {
        [[RCIMClient sharedRCIMClient].pushProfile

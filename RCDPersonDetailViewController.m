...
- (void)btnVideoCall:(id)sender {
  ...
  [[RCCall sharedRCCall] startSingleCall:self.friendInfo.userId
    ...
    self.timer = [NSTimer timerWithTimeInterval:2.0 target:self selector:@selector(checkCallState) userInfo:nil repeats:YES];
...
-(void)checkCallState{
    if([[[RCCall sharedRCCall] currentCallSession]callStatus]==RCCallActive){
        ...
        self.timer = nil;
        self.timer2 = [NSTimer timerWithTimeInterval:2.0 target:self selector:@selector(checkMoney) userInfo:nil repeats:YES];
...
{  
  ...
  self.subViews = NSDictionaryOfVariableBindings(_infoView,_conversationBtn,remarksView,_videoCallBtn);//Chris:去掉了audiobtn
...
//                             views:self.subViews]];
  ...
  [self.view addConstraints:[NSLayoutConstraint
                             constraintsWithVisualFormat:@"V:[_conversationBtn]-15-[_videoCallBtn]"//Chris:去掉了audiobtn
                             options:0
                             metrics:nil
                             views:self.subViews]];
...
//                             }];
    ...
    self.phoneNumberLabel.textColor = [UIColor colorWithRed:0 green:143/255.0 blue:255/255.0 alpha:1];
    self.phoneNumberLabel.font = [UIFont systemFontOfSize:14.f];
    NSString *urlStr = [NSString stringWithFormat:@"https://ask.vipjingjie.com/moblie/personalAike?userid=%@",self.friendInfo.userId];
...
-(void)updateText:(NSString *)text{
    self.phoneNumberLabel.text = [NSString stringWithFormat:@"%@爱可币/分钟",text];
}

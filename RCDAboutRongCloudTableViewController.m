...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = NSLocalizedStringFromTable(@"Version", @"RongCloudKit",nil);
    ...
    [[RCDUIBarButtonItem alloc] initContainImage:[UIImage imageNamed:@"navigator_btn_back"]
                                  imageViewFrame:CGRectMake(-6, 4, 10, 17)
                                     buttonTitle:NSLocalizedStringFromTable(@"Me", @"RongCloudKit",nil)
        ...
        case 0: {
            switch (indexPath.row) {
                ...
                case 1: {
                    ...
                    cell.leftLabel.text = NSLocalizedStringFromTable(@"Platform Introduction", @"RongCloudKit",nil);
                    return cell;
                }
                    break;
                ...
                case 2: {
                    [cell setCellStyle:DefaultStyle];
                    cell.leftLabel.text = NSLocalizedStringFromTable(@"Officail WebSite", @"RongCloudKit",nil);
                    return cell;
                }
                    break;
                ...
                case 3: {
                    ...
                    versionCell.leftLabel.text = NSLocalizedStringFromTable(@"MoneyCall Version", @"RongCloudKit",nil);
...
//    }
    if(indexPath.row == 1){
        NSString *str1 = [NSString stringWithFormat:@"https://ask.vipjingjie.com/moblie/version?field=1&userid=%@",[RCIM sharedRCIM].currentUserInfo.userId];
    ...
    if(indexPath.row == 2){
        NSURL *url = [NSURL URLWithString:@"http://www.gex.sg"];
        NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url];
        UIViewController *vc = [[UIViewController alloc]init];
        vc.view.frame = [UIScreen mainScreen].bounds;
        self.webView = [[WKWebView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width,self.view.frame.size.height)];
...
- (void)setPoweredView
{
    ...
    footerLabel.text = @"Powered by Diandianqu";
...
- (NSArray *)urls {
    if (!_urls) {
        NSArray *section0 =
        [NSArray arrayWithObjects:@"http://taoxinzi.diandianqu.com/",
         @"http://taoxinzi.diandianqu.com/introduce.html",
         @"http://taoxinzi.diandianqu.com/", nil];

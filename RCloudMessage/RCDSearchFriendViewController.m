...
- (void)viewDidLoad {
    ...
    [button setTitle:@"从通讯录加好友" forState:UIControlStateNormal];
    button.titleLabel.tintColor=[UIColor whiteColor];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.backgroundColor = [UIColor colorWithHexString:@"0099ff" alpha:1.0];
    [button addTarget:self action:@selector(getContact:) forControlEvents:UIControlEventTouchUpInside];
    button.layer.masksToBounds = YES;
    button.layer.cornerRadius = 5.f;
    button.frame = CGRectMake((self.view.frame.size.width-260)/2, 240, 260, 60);
...
- (void)viewDidAppear:(BOOL)animated{
    if([_tels count]!=0){
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"已发好友请求" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil,nil];
        [alert show];
        for(NSString *tel in _tels){
            [RCDHTTPTOOL searchUserByPhone:tel
...
//                                                                                             });
//                                                                                         }];
                                                  [RCDHTTPTOOL requestFriend:user.userId complete:^(BOOL result) {
...
- (void)getContact:(id)sender {
    PickContactViewController *pick = [PickContactViewController alloc];

...
//  self.animatedImagesView.delegate = self;
  ...
  [sendCodeButton setBackgroundColor:[[UIColor alloc] initWithRed:133 / 255.f
                                                            green:133 / 255.f
                                                             blue:133 / 255.f
                                                            alpha:1]];
  [sendCodeButton setTitle:NSLocalizedStringFromTable(@"Send Code", @"RongCloudKit",nil) forState:UIControlStateNormal];
  ...
  passwordTextField.attributedPlaceholder = [[NSAttributedString alloc]
      initWithString:NSLocalizedStringFromTable(@"New Password", @"RongCloudKit",nil)
          attributes:@{NSForegroundColorAttributeName : color}];
        ...
        success:^(id response) {
          if ([response[@"code"] intValue] == 200) {
            ...
            if ([response[@"result"] integerValue] == 0) {
              ...
              [hud hide:YES];
              _errorMsgLb.text = NSLocalizedStringFromTable(@"Phone number is not registered", @"RongCloudKit",nil);
            }
          }
        }
        failure:^(NSError *err){
      ...
      success:^(id response) {
        if ([response[@"code"] intValue] == 200) {
          ...
          [AFHttpTool resetPassword:userPwd
              ...
              success:^(id response) {
                if ([response[@"code"] intValue] == 200) {
                  _errorMsgLb.text = NSLocalizedStringFromTable(@"Modify Success", @"RongCloudKit",nil);
                  dispatch_after(
...
- (BOOL)checkContent {
  ...
  if (vCode.length == 0) {
    _errorMsgLb.text = NSLocalizedStringFromTable(@"Verification code can't be empty", @"RongCloudKit",nil);
    return NO;
  }
  return YES;
}
...
- (void)timeFireMethod {
  ...
  ((UILabel *)[self.view viewWithTag:vCodeTimerLabelTag]).text =
      [NSString stringWithFormat:@"%d", _seconds];
  if (_seconds == 0) {

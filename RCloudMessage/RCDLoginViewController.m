...
//  self.animatedImagesView.delegate = self;
  ...
  [registerHeadButton setTitle:NSLocalizedStringFromTable(@"Forgot Password", @"RongCloudKit",nil) forState:UIControlStateNormal];
  ...
  [forgetPswHeadButton setTitle:NSLocalizedStringFromTable(@"Sign up", @"RongCloudKit",nil) forState:UIControlStateNormal];
  ...
  [loginButton addTarget:self
    ...
    if([[self getPreferredLanguage] isEqualToString:@"zh"]){
        btnImgName = @"login_button";
    }else{
        btnImgName = @"login_button_en";
    }
  ...
  [registerButton setTitle:NSLocalizedStringFromTable(@"Forgot Password", @"RongCloudKit",nil) forState:UIControlStateNormal];
  ...
  [forgetPswButton setTitle:NSLocalizedStringFromTable(@"Sign up", @"RongCloudKit",nil) forState:UIControlStateNormal];
            ...
            [self loginRongCloud:userName
              ...
              failure:^(NSError *err) {
                dispatch_async(dispatch_get_main_queue(), ^{
                  [hud hide:YES];
                  NSLog(@"Token无效");
                  _errorMsgLb.text = NSLocalizedStringFromTable(@"Fail to connect to server", @"RongCloudKit",nil);
                });
              }];
        }
      }];
...
- (void)login:(NSString *)userName password:(NSString *)password {
  ...
  if (RC_NotReachable == status) {
    _errorMsgLb.text = NSLocalizedStringFromTable(@"Network error,please check", @"RongCloudKit",nil);
    return;
  } else {
    _errorMsgLb.text = @"";
  }
  ...
  if ([self validateUserName:userName userPwd:password]) {
    ...
    hud.labelText = NSLocalizedStringFromTable(@"Logining...", @"RongCloudKit",nil);
    [hud show:YES];
    ...
    [AFHttpTool loginWithPhone:userName
        ...
        success:^(id response) {
          if ([response[@"code"] intValue] == 200) {
            ...
            if (_errCode == 1000) {
              _errorMsgLb.text = NSLocalizedStringFromTable(@"Phone number or password wrong", @"RongCloudKit",nil);
            }
            [_pwdTextField shake];
          }
        }
        failure:^(NSError *err) {
          [hud hide:YES];
          _errorMsgLb.text = NSLocalizedStringFromTable(@"Sign in failure,please check the network", @"RongCloudKit",nil);
        }];
  } else {
    _errorMsgLb.text = NSLocalizedStringFromTable(@"Please check the phone number and password", @"RongCloudKit",nil);
  }
}
...
- (BOOL)validateUserName:(NSString *)userName userPwd:(NSString *)userPwd {
  ...
  if (userName.length == 0) {
    alertMessage = NSLocalizedStringFromTable(@"Nickname can't be empty", @"RongCloudKit",nil);
    return NO;
  } else if (userPwd.length == 0) {
    alertMessage = NSLocalizedStringFromTable(@"Password can't be empty", @"RongCloudKit",nil);
    return NO;
  }
...
- (void)onRCIMConnectionStatusChanged:(RCConnectionStatus)status {
  dispatch_async(dispatch_get_main_queue(), ^{
    if (status == ConnectionStatus_Connected) {
      ...
      self.errorMsgLb.text = NSLocalizedStringFromTable(@"Network error,please check", @"RongCloudKit",nil);
    } else if (status == ConnectionStatus_KICKED_OFFLINE_BY_OTHER_CLIENT) {
      self.errorMsgLb.text = NSLocalizedStringFromTable(@"Your account login in other dvice,you will be logout", @"RongCloudKit",nil);
    } else if (status == ConnectionStatus_TOKEN_INCORRECT) {
      NSLog(@"Token无效");
      self.errorMsgLb.text = NSLocalizedStringFromTable(@"Fail to connect to server", @"RongCloudKit",nil);

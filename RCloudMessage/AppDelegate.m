...
#define RONGCLOUD_IM_APPKEY @"qd46yzrfqhxdf" //online key
...
@interface AppDelegate () <RCWKAppInfoProvider,UIAlertViewDelegate>
...
//    UIStoryboard *meStoryboard = [UIStoryboard storyboardWithName:@"Launch Screen" bundle:nil];
//    LaunchViewController *launchVC = [meStoryboard instantiateViewControllerWithIdentifier:@"launch"];
//    RCDNavigationViewController *_navi = [[RCDNavigationViewController alloc]
//                                          initWithRootViewController:launchVC];
//    self.window.rootViewController = _navi;
  ...
  [[RCIM sharedRCIM] registerMessageType:[RCChrisMessage class]];
  //设置会话列表头像和会话页面头像
...
//  [[RCCallClient sharedRCCallClient] setVideoProfile:RC_VIDEO_PROFILE_480P];
                                                            ...
                                                            if ([response[@"code"] intValue] == 200) {
                                                                ...
                                                                [[RCIM sharedRCIM] connectWithToken:newToken
                                                                                            success:^(NSString *userId) {
                                                                                                ...
                                                                                                [self gotoLoginViewAndDisplayReasonInfo:NSLocalizedStringFromTable(@"Login Failure,please retry", @"RongCloudKit",nil)];
                                                                                            } tokenIncorrect:^{
                                                                                                [self gotoLoginViewAndDisplayReasonInfo:NSLocalizedStringFromTable(@"Fail to connect to server", @"RongCloudKit",nil)];
                                                                                                NSLog(@"Token无效");
                                                                                            }];
                                                            } else {
                                                                [self gotoLoginViewAndDisplayReasonInfo:NSLocalizedStringFromTable(@"Phone number or password wrong", @"RongCloudKit",nil)];
                                                            }
                                                        }
                                                        failure:^(NSError *err) {
  ...
  if (pushServiceData) {
    ...
    [[UINavigationBar appearance]
     setBarTintColor:[UIColor colorWithHexString:@"FFFFFF" alpha:1.0f]];
...
//    taokeParams.pid = @"mm_108515078_13772558_56094198"; //mm_XXXXX为你自己申请的阿里妈妈淘客pid
//    [[AlibcTradeSDK sharedInstance] setTaokeParams:taokeParams];
//
//    //设置全局的app标识，在电商模块里等同于isv_code
//    //没有申请过isv_code的接入方,默认不需要调用该函数
//    [[AlibcTradeSDK sharedInstance] setISVCode:@"1.0.6"];
//
//    // 设置全局配置，是否强制使用h5
...
- (void)onRCIMConnectionStatusChanged:(RCConnectionStatus)status {
  if (status == ConnectionStatus_KICKED_OFFLINE_BY_OTHER_CLIENT) {
    UIAlertView *alert = [[UIAlertView alloc]
            initWithTitle:NSLocalizedStringFromTable(@"Notice", @"RongCloudKit",nil)
                  message:NSLocalizedStringFromTable(@"Your account login in other dvice,you will be logout", @"RongCloudKit",nil)
                 delegate:nil
    ...
    [AFHttpTool getTokenSuccess:^(id response) {
      ...
      UIAlertView *alert = [[UIAlertView alloc]
                            initWithTitle:NSLocalizedStringFromTable(@"Notice", @"RongCloudKit",nil)
                            message:NSLocalizedStringFromTable(@"Your account has been banned", @"RongCloudKit",nil)
                            delegate:nil
                            cancelButtonTitle:NSLocalizedStringFromTable(@"Confirm", @"RongCloudKit",nil)
                            otherButtonTitles:nil, nil];
      [alert show];
      ...
      if ([url.host isEqualToString:@"safepay"]) {
          ...
          [[AlipaySDK defaultService] processOrderWithPaymentResult:url standbyCallback:^(NSDictionary *resultDic) {
              ...
              if([resultCode isEqualToString:@"9000"]){
                  UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedStringFromTable(@"Payment Success",@"RongCloudKit",nil)
                                                                      message:nil
                                                                     delegate:self
                                                            cancelButtonTitle:NSLocalizedStringFromTable(@"Confirm",@"RongCloudKit",nil)
                                                            otherButtonTitles:nil, nil];
                  ...
                  UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedStringFromTable(@"Payment Failure or Handling",@"RongCloudKit",nil)
                                                                      message:nil
                                                                     delegate:self
                                                            cancelButtonTitle:NSLocalizedStringFromTable(@"Confirm",@"RongCloudKit",nil)
                                                            otherButtonTitles:nil, nil];
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if(buttonIndex == 0){
        NSNotification *notification =[NSNotification notificationWithName:@"refreshMoneyPage" object:nil userInfo:nil];

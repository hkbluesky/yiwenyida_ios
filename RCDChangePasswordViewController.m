...
- (void)viewDidLoad {
  ...
  self.navigationItem.title = NSLocalizedStringFromTable(@"Password Changing", @"RongCloudKit",nil);
...
- (void)initialize
{
  self.oldPwdLabel = [self setLabel:NSLocalizedStringFromTable(@"Original Password", @"RongCloudKit",nil)];
  ...
  self.newsPwdLabel = [self setLabel:NSLocalizedStringFromTable(@"New Password", @"RongCloudKit",nil)];
  self.newsPwdView = [self setSubView];
  self.newsPwdTextField = [self setTextField:NSLocalizedStringFromTable(@"6-16Characters Case sensitive", @"RongCloudKit",nil)];
  ...
  self.confirmPwdLabel = [self setLabel:NSLocalizedStringFromTable(@"Confirm Password", @"RongCloudKit",nil)];
  self.confirmPwdView = [self setSubView];
  self.confirmPwdTextField = [self setTextField:NSLocalizedStringFromTable(@"6-16Characters Case sensitive", @"RongCloudKit",nil)];
...
- (void)setNavigationButton {
  ...
  [[RCDUIBarButtonItem alloc] initContainImage:[UIImage imageNamed:@"navigator_btn_back"]
                                imageViewFrame:CGRectMake(-6, 4, 10, 17)
                                   buttonTitle:NSLocalizedStringFromTable(@"Setting", @"RongCloudKit",nil)
...
- (void)saveNewPassword:(id)sender {
  ...
  if ([userPwd isEqualToString:self.oldPwdTextField.text]) {
    ...
    if (newPwdLength <6 || newPwdLength > 20) {
      [self AlertShow:NSLocalizedStringFromTable(@"6-16Characters Case sensitive", @"RongCloudKit",nil)];
    }
    else {
      if ([self.newsPwdTextField.text isEqualToString:self.confirmPwdTextField.text]) {
        ...
        [self AlertShow:NSLocalizedStringFromTable(@"Confirm password not the seem as the new Password", @"RongCloudKit",nil)];
      }
    }

  } else {
    [self AlertShow:NSLocalizedStringFromTable(@"Original Password incorrect", @"RongCloudKit",nil)];
  }
}
...
- (void)AlertShow:(NSString *)content {
  UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil
                                                  message:content
                                                 delegate:self
                                        cancelButtonTitle:NSLocalizedStringFromTable(@"Done", @"RongCloudKit",nil)
